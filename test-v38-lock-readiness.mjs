...
const board={protocol:'V38_LIVE_RESEARCH_BOARD_V1',date:'2026-09-21',generated_at:'2026-09-21T21:00:00Z',profile_snapshot_sha256:'a'.repeat(64),point_in_time:true,research_only:true,rows:[
  ...
  row(2,{...base,blast:7},{gate_count:5,confirmed_lineup:false,lineup:null,final_review_queue:false}),
...
assert.deepEqual(z.counts,{review_rows:4,actionable_review:2,pending_lineup:0,price_blocked_4of6:1,policy_blocked:1});
assert.equal(z.qualification_population.locked_6of6,1);assert.equal(z.qualification_population.locked_5of6,1);assert.equal(z.qualification_population.protected_4of6_700plus,1);assert.equal(z.qualification_population.price_unknown_4of6,1);assert.equal(z.qualification_population.locked_qualified_total,3);
assert.equal(z.compression_diagnostics.review_queue_6of6,1);assert.equal(z.compression_diagnostics.review_queue_5of6,0);assert.equal(z.compression_diagnostics.locked_5of6_outside_review_queue,1);assert.equal(z.compression_diagnostics.profile_only_saturation,false);
...
assert.equal(z.rows.some(r=>r.player_id===2),false);
...
const satBoard={...board,rows:[row(10,base,{gate_count:6}),row(11,{...base,blast:7},{gate_count:5,final_review_queue:false})]};const sat=buildLockReadiness(satBoard);assert.equal(sat.compression_diagnostics.profile_only_saturation,true);assert.match(sat.compression_diagnostics.warning,/6OF6_ONLY/);
