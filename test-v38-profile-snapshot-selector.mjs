...
function snap({captured='2026-09-03T14:00:00.000Z',gameCount=1,excludedCount=0,profileComplete=1}={}){const starts=Array.from({length:gameCount},(_,i)=>({gamePk:i+1,start_time:'2026-09-03T23:00:00.000Z'})),excluded=Array.from({length:excludedCount},(_,i)=>({gamePk:100+i,start_time:'2026-09-03T13:00:00.000Z'}));return sign({snapshot_protocol:V38_PROFILE_SNAPSHOT_PROTOCOL,date:'2026-09-03',captured_at:captured,point_in_time:true,research_only:true,scoring_enabled:false,scoring_eligible:false,model_scoring_changed:false,pregame_games:starts,excluded_started_games:excluded,profile_complete:profileComplete,items:[{player_id:1}]})}
...
const fullEarly=snap({captured:'2026-09-03T14:11:00.000Z',gameCount:9,profileComplete:260}),shrunkLate=snap({captured:'2026-09-03T20:00:00.000Z',gameCount:7,excludedCount:2,profileComplete:280});
...
if(V38_POSTGAME_PROFILE_SELECTION.selection_rule!=='require zero excluded started games; then maximize verified pregame game count; then profile_complete; then latest captured_at')throw Error('postgame selection rule drift');
