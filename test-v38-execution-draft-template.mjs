...
assert.equal(z.protocol,EXECUTION_DRAFT_TEMPLATE_PROTOCOL);assert.equal(z.protocol,'BANDALYTICS_EXECUTION_DRAFT_TEMPLATE_V2');assert.equal(z.decision_state,'UNRESOLVED');assert.equal(z.rows.length,2);assert.equal(z.rows[0].eligible_for_final_cut,true);assert.equal(z.rows[1].eligible_for_final_cut,false);assert.equal(z.rows[0].final_cut,null);assert.equal(z.rows[0].exposure_state,'UNCLASSIFIED');assert.deepEqual(z.rows[0].evidence_lanes,[]);assert.deepEqual(z.tickets,[]);assert.equal(z.source_readiness_sha256,readiness.sha256);assert.equal(z.source_compression_watch_sha256,watch.sha256);assert.equal(z.compression_watch.length,1);assert.equal(z.compression_watch[0].player,'C');assert.equal(z.compression_watch[0].review_disposition,null);assert.equal(z.compression_watch[0].auto_promote,false);assert.match(z.sha256,/^[a-f0-9]{64}$/);
