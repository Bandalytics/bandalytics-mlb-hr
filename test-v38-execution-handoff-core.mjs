...
assert.equal(z.summary.qualified.n,4);
assert.equal(z.summary.final_pool.n,4);
assert.equal(z.summary.lineup_eligible_final_pool.n,3);
assert.equal(z.summary.ticketed_eligible.n,2);
assert.equal(z.summary.zero_path_count,1);
assert.equal(z.summary.opportunity_mismatch_count,1);
assert.equal(z.summary.total_ticket_paths,4);
assert.equal(z.summary.top3_exposure_concentration_pct,100);
assert.equal(z.summary.max_single_hitter_portfolio_dependency_pct,75);
