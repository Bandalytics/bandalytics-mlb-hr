...
export function attachProspectiveModifierBands(row,sets,{date,gamePk,startTime,matchup}={}){const pArt=selectLatestProspectiveModifier(sets?.pitchfitArtifacts||[],{date,gamePk,startTime,type:'pitchfit'}),bArt=selectLatestProspectiveModifier(sets?.bbeArtifacts||[],{date,gamePk,startTime,type:'bbe'}),p=rowForGamePlayer(pArt,{gamePk,playerId:row.player_id,matchup}),b=rowForGamePlayer(bArt,{gamePk,playerId:row.player_id,matchup}),pBands=buildPitchfitBands(pArt?.rows||[]),bBands=buildBbeBands(bArt?.rows||[]),pitchfitVerified=!!pArt,bbeVerified=!!bArt,anyVerified=pitchfitVerified||bbeVerified;return{...row,pitchfit:p,pitchfit_band:pBands.classify(p),bbe:b,bbe_band:bBands.classify(b),modifier_evidence:{pitchfit_captured_at:pArt?.captured_at||null,pitchfit_snapshot_sha256:pArt?.sha256||null,pitchfit_cryptographically_verified:pitchfitVerified,bbe_captured_at:bArt?.captured_at||null,bbe_snapshot_sha256:bArt?.sha256||null,bbe_cryptographically_verified:bbeVerified,pitchfit_population:pBands.population||0,bbe_population:bBands.population||0,cryptographically_verified:anyVerified,per_game_strictly_pregame:true}};}
