...
export function attachProspectiveModifierBands(row,sets,{date,gamePk,startTime,matchup}={}){const pArt=selectLatestProspectiveModifier(sets?.pitchfitArtifacts||[],{date,gamePk,startTime,type:'pitchfit'}),bArt=selectLatestProspectiveModifier(sets?.bbeArtifacts||[],{date,gamePk,startTime,type:'bbe'}),p=rowForGamePlayer(pArt,{gamePk,playerId:row.player_id,matchup}),b=rowForGamePlayer(bArt,{gamePk,playerId:row.player_id,matchup}),pBands=buildPitchfitBands(pArt?.rows||[]),bBands=buildBbeBands(bArt?.rows||[]);return{...row,pitchfit:p,pitchfit_band:pBands.classify(p),bbe:b,bbe_band:bBands.classify(b),modifier_evidence:{pitchfit_captured_at:pArt?.captured_at||null,pitchfit_snapshot_sha256:pArt?.sha256||null,bbe_captured_at:bArt?.captured_at||null,bbe_snapshot_sha256:bArt?.sha256||null,pitchfit_population:pBands.population||0,bbe_population:bBands.population||0,cryptographically_verified:true,per_game_strictly_pregame:true}};}
...
export function prospectiveModifierCoverage(rows=[],sets={}){return{pitchfit_artifacts_loaded:(sets.pitchfitArtifacts||[]).length,bbe_artifacts_loaded:(sets.bbeArtifacts||[]).length,rows_with_true_pitchfit:rows.filter(r=>r.pitchfit?.fit_status==='TRUE').length,rows_with_full_bbe:rows.filter(r=>+r.bbe?.tracked_bbe>=15).length,rows_pitchfit_top_quartile:rows.filter(r=>['TOP_QUARTILE','TOP_DECILE'].includes(r.pitchfit_band)).length,rows_bbe_hrshape_top_quartile:rows.filter(r=>['TOP_QUARTILE','TOP_DECILE'].includes(r.bbe_band?.hrshape_band)).length,rows_with_verified_modifier_evidence:rows.filter(r=>r.modifier_evidence?.cryptographically_verified===true).length,rows_with_per_game_modifier_evidence:rows.filter(r=>r.modifier_evidence?.per_game_strictly_pregame===true).length};}
