...
function simulateGame(config={}) {
  ...
  const scoreGrid=includeScoreGrid?Object.fromEntries([...scoreMap.entries()].filter(([key])=>{const [a,h]=key.split('-').map(Number);return a<=scoreGridMax&&h<=scoreGridMax}).map(([key,count])=>[key,{probability:count/N,pct:100*count/N,fairAmerican:americanOddsFromProb(count/N)}])):undefined;
...
function runBandalyticsSim(matchup={}) {
  ...
  const sim = simulateGame({
    ...
    includeScoreGrid: !!matchup.includeScoreGrid,
    scoreGridMax: matchup.scoreGridMax || 12,
  });
  return {estimates, sim};
}


export { LEAGUE, estimateTeamRuns, estimateGameRuns, simulateGame, runBandalyticsSim, americanOddsFromProb, impliedProbFromAmerican, evPerUnit };
