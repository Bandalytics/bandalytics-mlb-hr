...
function evidenceLanes(v){
  const a=Array.isArray(v)?v:[];
  return [...new Set(a.map(x=>String(x||'').trim().toUpperCase().replace(/[ -]+/g,'_')).filter(x=>EVIDENCE_LANES.includes(x)))];
}
...
export function evaluateExecutionHandoff(rows=[],executionPlan=[]){
  ...
  for(const p of executionPlan||[]){const g=n(p?.gamePk??p?.game_pk),pid=n(p?.player_id);if(g==null||pid==null)continue;planMap.set(idKey(g,pid),p)}
  const out=[];
  for(const row of rows||[]){
    ...
    const state=exposureState(p?.exposure_state),lanes=evidenceLanes(p?.evidence_lanes);
    ...
    const zeroPathReason=p?.zero_path_reason||null;
    ...
    out.push({...row,execution:{policy,lineup,final_cut:finalCut,explicit_cut:explicitCut,cut_reason:cutReason,zero_path_reason:zeroPathReason,exposure_state:state,evidence_lanes:lanes,evidence_lane_count:lanes.length,ticket_paths:ticketPaths,ticket_ids:ids,ticketed,planned_exposure:plannedExposure,opportunity_mismatch:opportunityMismatch,zero_path_qualified:zeroPathQualified,intentional_zero:intentionalZero,unclassified_qualified:unclassifiedQualified,plan_hr_odds:n(p?.hr_odds??p?.odds??p?.american_odds)}});
  }
  ...
  const top3=usage.slice(0,3).reduce((a,b)=>a+b,0),top3Concentration=totalLegs?top3/totalLegs:null;
  ...
  const byLaneCount={};for(let i=0;i<=4;i++)byLaneCount[String(i)]=summarize(finalPool.filter(r=>r.execution.evidence_lane_count===i));
  const ticketedByLaneCount={};for(let i=0;i<=4;i++)ticketedByLaneCount[String(i)]=summarize(ticketed.filter(r=>r.execution.evidence_lane_count===i));
  ...
  const cut=qualified.filter(r=>r.execution.explicit_cut),cutHr=cut.filter(r=>r.homer===true).length;
  return {
    ...
    summary:{
      ...
      total_ticket_paths:totalLegs,unique_ticket_count:allTicketIds.size||null,
      ...
      by_exposure_state:byExposure,by_evidence_lane_count:byLaneCount,ticketed_by_evidence_lane_count:ticketedByLaneCount,by_evidence_lane_presence:lanePresence
    }
  };
}
