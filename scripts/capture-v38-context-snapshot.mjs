...
const lineupRows=(feed.lineup_players||[]).filter(x=>gamePks.has(+x.gamePk)&&matchups.has(String(x.matchup||''))).map(x=>({gamePk:+x.gamePk,player_id:+x.player_id,player:x.player||null,team:x.team||null,game_matchup:x.matchup||null,lineup:x.lineup??null,opp_pitcher_id:x.opp_pitcher_id??null,opp_pitcher:x.opp_pitcher??null,opp_pitcher_hand:x.opp_pitcher_hand??null,bat_side:x.bat_side??null}));
