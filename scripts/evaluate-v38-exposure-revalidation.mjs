...
if(!path) throw Error('Usage: node scripts/evaluate-v38-exposure-revalidation.mjs <workflow-revalidation.json>');
...
const totalQualifiedHr=eligible.filter(r=>r.homer===true).length;
const results={};
for(const [name,keyFn] of Object.entries(strategies)){
  ...
  results[name]={};
  for(const n of sizes){const selected=ranked.slice(0,n);results[name][`TOP_${n}`]={...cohort(selected),qualified_hr_capture_pct:totalQualifiedHr?Number((100*selected.filter(r=>r.homer===true).length/totalQualifiedHr).toFixed(2)):null};}
}
const out={protocol:'V38_EXPOSURE_REVALIDATION_V1',date:input.date,point_in_time:true,as_of_verified:true,forward_leakage_days:0,research_only:true,scoring_enabled:false,ranking_contract:'LEXICOGRAPHIC_PREDECLARED_NO_OUTCOME_INPUT',candidate_source:'ANTI_OVERCOMPRESSION',price_provenance_status:input.price_provenance_status||'UNAVAILABLE_IN_REPLAY_INPUTS',roi_status:'UNAVAILABLE_NOT_FABRICATED',eligible:cohort(eligible),strategies:results};
const outPath=`snapshots/v38-exposure-revalidation-${input.date}.json`;
fs.writeFileSync(outPath,JSON.stringify(out,null,2)+'\n');
console.log(`V38_EXPOSURE_REVALIDATION_PATH=${outPath}`);
console.log(`V38_EXPOSURE_REVALIDATION_SUMMARY=${JSON.stringify(out)}`);
