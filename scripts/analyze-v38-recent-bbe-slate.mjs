...
const captured_at=new Date().toISOString(),requested=process.argv[2],date=/^2026-\d\d-\d\d$/.test(String(requested||''))?requested:etDate(),feed=await buildNativeFeed({date,timeoutMs:16000}),lineups=(feed.lineup_players||[]).filter(x=>Number.isInteger(+x.player_id));
...
const trends=usable.reduce((o,x)=>{const k=x.bbe.trend||'UNKNOWN';o[k]=(o[k]||0)+1;return o},{}),out={protocol:'V38_RECENT_BBE_DISTRIBUTION_V1',date,captured_at,capture_mode:'AS_OF_RECONSTRUCTABLE',research_only:true,scoring_enabled:false,scoring_eligible:false,model_scoring_changed:false,as_of_verified:true,as_of_rule:'Recent BBE query excludes same-day events and includes the prior day, validated on 2026-09-02.',lineup_rows:lineups.length,summary_rows:summaries.length,usable_rows:usable.length,full_15_bbe_rows:full.length,trend_counts:trends,quantiles,top_hrshape:[...usable].sort((a,b)=>(b.bbe?.hrshape||0)-(a.bbe?.hrshape||0)).slice(0,25).map(x=>({player_id:x.player_id,player:x.player,team:x.team,tracked_bbe:x.tracked_bbe,...x.bbe})),rows:summaries};
await fs.mkdir('snapshots',{recursive:true});const path=`snapshots/v38-recent-bbe-${date}.json`;await fs.writeFile(path,JSON.stringify(out,null,2)+'\n');console.log(`V38_BBE_PATH=${path}`);console.log(`V38_BBE_SUMMARY=${JSON.stringify({date,captured_at:out.captured_at,capture_mode:out.capture_mode,lineup_rows:out.lineup_rows,usable_rows:out.usable_rows,full_15_bbe_rows:out.full_15_bbe_rows,trend_counts:out.trend_counts,quantiles:out.quantiles,top_hrshape:out.top_hrshape.slice(0,10)})}`);
