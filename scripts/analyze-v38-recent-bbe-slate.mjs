...
await fs.mkdir('snapshots',{recursive:true});const stamp=captured_at.replaceAll(':','').replaceAll('.',''),path=liveProspective?`snapshots/v38-recent-bbe-${date}-${stamp}.json`:`snapshots/v38-recent-bbe-${date}.json`;await fs.writeFile(path,JSON.stringify(out,null,2)+'\n');console.log(`V38_BBE_PATH=${path}`);console.log(`V38_BBE_SUMMARY=${JSON.stringify({date,captured_at:out.captured_at,capture_mode:out.capture_mode,prospective_pregame_only:out.prospective_pregame_only,pregame_games:out.pregame_game_pks.length,lineup_rows:out.lineup_rows,usable_rows:out.usable_rows,full_15_bbe_rows:out.full_15_bbe_rows,trend_counts:out.trend_counts,quantiles:out.quantiles,top_hrshape:out.top_hrshape.slice(0,10)})}`);