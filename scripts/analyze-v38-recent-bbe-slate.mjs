...
const captured_at=new Date().toISOString(),requested=process.argv[2],date=/^2026-\d\d-\d\d$/.test(String(requested||''))?requested:etDate(),liveProspective=date===etDate(),feed=await buildNativeFeed({date,timeoutMs:16000});let allowedGamePks=null,pregame_game_pks=[];if(liveProspective){const schedule=await get(`${MLB}/api/v1/schedule?sportId=1&date=${encodeURIComponent(date)}&hydrate=team`),now=Date.parse(captured_at),games=(schedule.dates?.[0]?.games||[]).filter(g=>Number.isFinite(Date.parse(g.gameDate))&&Date.parse(g.gameDate)>now);pregame_game_pks=games.map(g=>+g.gamePk);allowedGamePks=new Set(pregame_game_pks)}
const lineups=(feed.lineup_players||[]).filter(x=>Number.isInteger(+x.player_id)&&Number.isInteger(+x.gamePk)).filter(x=>!allowedGamePks||allowedGamePks.has(+x.gamePk));
const byGamePlayer=new Map(lineups.map(x=>[`${+x.gamePk}:${+x.player_id}`,x])),uniqueIds=[...new Set(lineups.map(x=>+x.player_id))],baseById=new Map(),summaries=[];
for(const batch of chunks(uniqueIds,20)){
  try{
    const{url}=buildSavantBbeUrl({ids:batch,date}),csv=await fetchText(url,{timeoutMs:25000}),z=summarizeBbeCsv(csv,{ids:batch,date});
    for(const x of z)baseById.set(+x.player_id,x);
  }catch(e){for(const id of batch)baseById.set(id,{player_id:id,tracked_bbe:0,bbe:null,error:e.message})}
}
for(const p of lineups){const x=baseById.get(+p.player_id)||{player_id:+p.player_id,tracked_bbe:0,bbe:null,error:'MISSING_BBE_SUMMARY'};summaries.push({...x,gamePk:+p.gamePk,player:p.player||null,team:p.team||null,lineup:p.lineup??null,matchup:p.matchup||null})}
...
const trends=usable.reduce((o,x)=>{const k=x.bbe.trend||'UNKNOWN';o[k]=(o[k]||0)+1;return o},{}),body={protocol:'V38_RECENT_BBE_DISTRIBUTION_V1',date,captured_at,capture_mode:'AS_OF_RECONSTRUCTABLE',prospective_pregame_only:liveProspective,pregame_game_pks,research_only:true,scoring_enabled:false,scoring_eligible:false,model_scoring_changed:false,as_of_verified:true,row_identity:'GAMEPK_PLAYER_ID',as_of_rule:'Recent BBE query excludes same-day events and includes the prior day, validated on 2026-09-02.',lineup_rows:lineups.length,summary_rows:summaries.length,usable_rows:usable.length,full_15_bbe_rows:full.length,trend_counts:trends,quantiles,top_hrshape:[...usable].sort((a,b)=>(b.bbe?.hrshape||0)-(a.bbe?.hrshape||0)).slice(0,25).map(x=>({gamePk:x.gamePk,player_id:x.player_id,player:x.player,team:x.team,tracked_bbe:x.tracked_bbe,...x.bbe})),rows:summaries};
