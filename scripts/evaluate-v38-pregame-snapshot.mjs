...
function teamGames(snapshot,teamId){return(snapshot.pregame_games||[]).filter(g=>+g.away_team_id===+teamId||+g.home_team_id===+teamId)}
function hitterMatchup(meta,gctx,teamId){if(!gctx?.game||!meta)return null;return +meta.away_team_id===+teamId?`${gctx.game.away} @ ${gctx.game.home}`:`${gctx.game.home} @ ${gctx.game.away}`}
...
if(pendingGames.length){
  ...
  for(const p of snapshot.items||[]){
    ...
    for(const meta of teamGames(snapshot,teamId)){
      const gamePk=+meta.gamePk;if(!playedByGame.get(gamePk)?.has(pid))continue;
      const selected=selectLatestPregameContext(contextSnapshots,gamePk,meta.start_time),gctx=selected?contextForGame(selected,gamePk):null,market=gctx?.market_rows?.find(x=>+x.player_id===pid)||null,lineup=gctx?.lineup_rows?.find(x=>+x.player_id===pid)||null;
      ...
      let row={row_identity:'GAMEPK_PLAYER_ID',player_id:pid,player:p.player||null,team_id:teamId,gamePk,homer:hrByGame.get(gamePk)?.has(pid)||false,profile_complete:profileComplete(p),ev:p.ev??null,hh:p.hh??p.hard_hit??null,barrel:p.barrel??null,iso:p.iso??null,pullair:p.pullair??p.pull_air??null,blast:p.blast??p.blast_swing??p.blasts_swing??null,sweet:p.sweet??p.sweet_spot??null,longshot_profile:g.longshot_profile,foundation_boost:g.foundation_boost,profile_pass:g.pass,profile_score:candidatePowerScore(p),gate_count:candidate.gate_count,gate_passes:candidate.passes,candidate_rules:candidate.rules,context};
      row=attachProspectiveModifierBands(row,modifierSets,{date:snapshot.date,gamePk,startTime:meta.start_time,matchup:hitterMatchup(meta,gctx,teamId)});rows.push(row);
    }
  }
