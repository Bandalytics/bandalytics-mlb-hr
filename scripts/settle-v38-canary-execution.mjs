...
if(!freeze.frozen_at||!Number.isFinite(Date.parse(freeze.frozen_at))) throw Error('freeze missing frozen_at');
if(!freeze.sha256) throw Error('freeze missing sha256');
...
if(Date.parse(outcomes.settled_at)<=Date.parse(freeze.frozen_at)) throw Error('settlement must occur after execution freeze');
...
const output={protocol:'V38_CANARY_SETTLEMENT_V1',date:freeze.date,settled_at:outcomes.settled_at,canary_only:true,production_normal_volume:false,source_freeze_sha256:computedFreezeSha,tickets:tickets.length,winning_tickets:wins,ticket_win_rate_pct:tickets.length?+(100*wins/tickets.length).toFixed(2):0,total_stake_units:totalStake,gross_return_units:gross,net_units:net,realized_roi_pct:roi,unique_ticketed_hitters:ticketedIds.length,ticketed_hr:ticketedHr,ticketed_hr_rate_pct:ticketedIds.length?+(100*ticketedHr/ticketedIds.length).toFixed(2):0,tickets_detail:tickets,roi_status:'REALIZED_FROM_VERIFIED_FROZEN_PRICE_AND_STAKE',notes:['Settlement recomputes and verifies the execution-freeze SHA-256 before using any frozen ticket data.','ROI uses only pregame frozen prices and pregame frozen stake_units from the canary execution freeze.','Outcome rows only determine win/loss; they cannot alter ticket membership, price, or stake.','This settlement remains canary evidence and does not independently enable normal-volume production.']};
