...
const MARKET_SCHEMA='BANDALYTICS_MLB_HR_MARKET_CURRENT_V1';
...
const marketById=new Map();for(const m of markets){const id=Number(m.player_id),valid=m.identity_status==='EXACT'&&m.market_schema===MARKET_SCHEMA&&m.market_type==='MLB_BATTER_HOME_RUN_YES';if(Number.isInteger(id)&&valid)marketById.set(id,m)}
...
for(const r of rows){
  ...
  const hrOddsSource=useDraft?'EXECUTION_DRAFT_EXPLICIT':useMarket?'FROZEN_CONTEXT_MARKET':null,hrOddsFrozenAt=hrOdds==null?null:hydratedAt;
