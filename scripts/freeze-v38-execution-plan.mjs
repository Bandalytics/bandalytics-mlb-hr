...
for(const r of rows){
  ...
  for(const lane of lanes)laneCounts[lane]=(laneCounts[lane]||0)+1;
  frozenRows.push({...r,exposure_state:state,evidence_lanes:lanes,ticket_paths:paths,ticket_ids:tickets.map((t,i)=>t.some(x=>key(x)===k)?`T${i+1}`:null).filter(Boolean)});
}
...
const body={
  protocol:'BANDALYTICS_EXECUTION_PLAN_PROSPECTIVE_V2',
  ...
  summary:{
    final_pool_n:finalPool,ticketed_n:ticketed,intentional_zero_n:intentionalZero,zero_path_n:intentionalZero,
