...
export default async function handler(req,res){
  ...
  if(!key) return res.status(503).json({ok:false,error:'MARKET_KEY_REQUIRED',provider:'SPORTSGAMEODDS',live_market_connected:false,ready_to_connect:true,required_env:'SPORTSGAMEODDS_API_KEY',market:'MLB batter home run yes',identity_mode:'MLBAM_EXACT_FAIL_CLOSED',slate_isolation:'MLB_GAME_TIME_MATCH',model_scoring_changed:false});
  try{
