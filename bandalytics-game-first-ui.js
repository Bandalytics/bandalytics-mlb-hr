...
function decorateProfiles(root){for(const row of qsa('.bd-line,.bd-look',root)){const text=(row.textContent||'').replace(/\s+/g,' ').trim();row.classList.remove('bd-profile-qualified','bd-profile-longshot');row.removeAttribute('data-bd-profile');let badge='';if(/\b6\s*\/\s*6\b/i.test(text))badge='6/6';else if(/\b5\s*\/\s*6\b/i.test(text))badge='5/6';else if((/\b4\s*\/\s*6\b/i.test(text)&&/(longshot|\+700|ls\b)/i.test(text))||/ls\s*4\s*\/\s*6/i.test(text))badge='LS 4/6';if(!badge)continue;const longshot=badge==='LS 4/6'||/(longshot|\+700|ls\b)/i.test(text);row.classList.add(longshot?'bd-profile-longshot':'bd-profile-qualified');row.setAttribute('data-bd-profile',badge);const evidence=qs('small',row);if(evidence)evidence.classList.add('bd-evidence-strip');}}
...
function observe(){const p=panel();if(!p)return;let queued=false;const mo=new MutationObserver(()=>{if(queued)return;queued=true;queueMicrotask(()=>{queued=false;markReady()})});mo.observe(p,{childList:true,subtree:true,characterData:true});markReady()}
...
window.__BANDALYTICS_GAME_FIRST_UI={version:'V1.1',publicTakeover:true,hidesDiagnosticReadiness:true,profileHighlighting:'presentation-only',modelScoringChanged:false,profileGateChanged:false,longshotRuleChanged:false,fullLineupVisible:true};
})();
