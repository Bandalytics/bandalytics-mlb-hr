...
const missingLineup=seal({...context,lineup_rows:[context.lineup_rows[0]]});await fs.writeFile('/tmp/hyd-missing-context.json',JSON.stringify(missingLineup));
...
const dupProfile=seal({...profile,items:[...profile.items,{player_id:303,player:'Alpha Batter',team_id:3}]});await fs.writeFile('/tmp/hyd-dup-profile.json',JSON.stringify(dupProfile));
