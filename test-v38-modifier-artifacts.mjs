...
import{rowForGamePlayer}from'./v38-prospective-modifier-selector.mjs';

function signed(protocol,rows,{date='2026-09-04',pregame_game_pks=[1]}={}){
  const base={protocol,date,captured_at:`${date}T16:00:00.000Z`,capture_mode:'AS_OF_RECONSTRUCTABLE',prospective_pregame_only:true,pregame_game_pks,row_identity:'GAMEPK_PLAYER_ID',research_only:true,scoring_enabled:false,scoring_eligible:false,model_scoring_changed:false,as_of_verified:true,as_of_rule:'test',lineup_rows:rows.length,rows};
...
const pf=signed('V38_PITCHFIT_DISTRIBUTION_V1',[{gamePk:1,player_id:1,matchup:'A @ B',fit_status:'TRUE',fit_score:55}]);
const bb=signed('V38_RECENT_BBE_DISTRIBUTION_V1',[{gamePk:1,player_id:1,matchup:'A @ B',tracked_bbe:15,bbe:{hrshape:10}}]);
...
const missingIdentity=signed('V38_PITCHFIT_DISTRIBUTION_V1',[{gamePk:1,player_id:1,matchup:'A @ B',fit_status:'TRUE',fit_score:55}]);delete missingIdentity.row_identity;const{sha256:_old,...missingBody}=missingIdentity;missingIdentity.sha256=crypto.createHash('sha256').update(JSON.stringify(missingBody)).digest('hex');assert.equal(validModifierArtifact(missingIdentity,{date:'2026-09-04'}),false);
const wrongGameRow=signed('V38_RECENT_BBE_DISTRIBUTION_V1',[{gamePk:2,player_id:1,matchup:'A @ B',tracked_bbe:15,bbe:{hrshape:10}}],{pregame_game_pks:[1]});assert.equal(validModifierArtifact(wrongGameRow,{date:'2026-09-04'}),false);
const legacy=signed('V38_PITCHFIT_DISTRIBUTION_V1',[{player_id:1,matchup:'A @ B',fit_status:'TRUE',fit_score:55}],{date:'2026-09-02',pregame_game_pks:[1]});delete legacy.row_identity;const{sha256:_legacy,...legacyBody}=legacy;legacy.sha256=crypto.createHash('sha256').update(JSON.stringify(legacyBody)).digest('hex');assert.equal(validModifierArtifact(legacy,{date:'2026-09-02'}),true);

const sameMatchupTwoGames=signed('V38_PITCHFIT_DISTRIBUTION_V1',[{gamePk:1,player_id:7,matchup:'A @ B',fit_status:'TRUE',fit_score:50},{gamePk:2,player_id:7,matchup:'A @ B',fit_status:'TRUE',fit_score:70}],{pregame_game_pks:[1,2]});
assert.equal(rowForGamePlayer(sameMatchupTwoGames,{gamePk:1,playerId:7,matchup:'A @ B'}).fit_score,50);
assert.equal(rowForGamePlayer(sameMatchupTwoGames,{gamePk:2,playerId:7,matchup:'A @ B'}).fit_score,70);
assert.equal(rowForGamePlayer(sameMatchupTwoGames,{gamePk:3,playerId:7,matchup:'A @ B'}),null);
